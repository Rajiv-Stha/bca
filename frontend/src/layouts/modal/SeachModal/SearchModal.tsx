import {
  ...
  const  SearchModal:React.FC<SearchModalPropsType>=({children,full})=> {
    ...
    return (
      <>
        <span onClick={onOpen} style={{width:full ? "100%":"32%"}}>{children}</span>
  
        <Modal isOpen={isOpen} onClose={onClose}>
          <ModalOverlay />
          <ModalContent>
      <div className={styles.searchContainer}>
              <input className={styles.searchInput} type="text" onChange={handleInputChange}  placeholder='search  in thrift treasure' value={searchInput}/>
          {
          searchInput.length > 0 &&     searchedProduct.map(product=>(
              <Link to={`/product/${product._id}`} className={styles.searchBox}>
