  ...
  return (
    <div className={styles.profile_card}>
      ...
      <div className={styles.avatar_box}>
        <input type="file" ref={imageRef}  style={{display:"none"}} onChange={handleChangeFile}/>
        <img src={ imageFile ? URL.createObjectURL(imageFile): user?.image} alt="avatar" className={styles.avatar_logo} />
        <div className={styles.avatar_btn}>
            <button className={styles.avatar_header_btns} onClick={()=>imageRef.current?.click()}>
                <HiOutlinePhotograph/>
                <p>Change</p>
                </button>
...
<select value={profileData.country} onChange={handleSelectChange}  >
    ...
    <option value="Bangladesh">Bangladesh</option>
        ...
        <input type="text" name="about" value={profileData.about} onChange={handleInputChange}/>    
        </div>
      </form>
      <div className={styles.btn_wrapper}>
        <button className={styles.cancel_btn}>Cancel</button>
        <button className={styles.update_btn} onClick={handleUpdateUser}>Update</button>
      </div>
    </div>
  )
}
